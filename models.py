...
class Ship(GImage):
    ...
    def __init__(self, **keywords):
        super().__init__(x = 400 , y = 54 , width = SHIP_WIDTH , height = SHIP_HEIGHT , source = 'ship.png')
...
class Bolt(GRectangle):
    ...
    def __init__(self, X, Y, height, direction, fillcolor ):
        self._velocity = direction
        self._bolts = []
        super().__init__(x = X,y = Y, width = BOLT_WIDTH, height = BOLT_HEIGHT,
        velocity = self._velocity, fillcolor = "red", linecolor = "blue")
    ...
    def isPlayerBolt(self):
        """
        Checks the velocity of a bolt.If the velocity is positive, the bolt is a players bolt,
        If the velocity is negative, the bolt is an Alien Bolt.
        """
